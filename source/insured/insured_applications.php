...
{	
	...
	if(!isset($error_message))
	{
		if(isset($login_session)){
			...
			@require_once("../utility/dbcontroller.php");
			...
			if($result <= 1)
			{
				...
				$query = "UPDATE users SET insured_children = insured_children+1 WHERE username='".$login_session."' " ;
				$result = $db_handle->updateQuery($query);
				$query = "UPDATE users SET name = '" . $_POST["firstName"] . "', surname = '" . $_POST["lastName"] . "', email = '" . $_POST["userEmail"] . "', AFM = '" . $_POST["AFM"] . "',AMKA = '" . $_POST["AMKA"] . "',phone = '" . $_POST["phone"] . "',city = '" . $_POST["city"] . "',region = '" . $_POST["region"] . "',address = '" . $_POST["address"] . "' WHERE username='".$login_session."' " ;
				$result = $db_handle->updateQuery($query);
				
				if(!empty($result))
				{
					$error_message = "";
					$success_message = "Αίτηση και ενημέρωση στοιχείων επιτυχής!";	
					unset($_POST);
				}
				else 
			...
			$success_message = "Αίτηση επιτυχής!";	
	}
}
?>
...
<div class="wrapper row2">
  <div id="container" class="clear">
	...
	<a href="./insured_applications.php" style="float:left; margin-bottom: 20px; text-decoration: none;" >&nbsp>Αίτηση Ασφάλισης Τέκνου</a>
	
     <!-- main content (Slider sto CSS)-->
    <section id="slider" class="clear">
      <figure>
        <figcaption style="height:1230px;">
			<h2><b>Αίτηση Ασφάλισης Τέκνου</b></h2>
			<form name="frmRegistration" method="post" action="">
				<table border="0" width="500" align="center" class="demo-table">
					...
					if(isset($login_session)){
						...
						echo '<form action="../utility/edit_profile.php" method="post">';
						...
						echo '<td> <input type="email" name="userEmail" class="demoInputBox" value="';
						...
						?>
						<tr>
						<td style="padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;">*Παρακαλώ να εγκρίνετε την:</td>
						<td style="padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;">
						<input type="radio" required> Ασφάλιση<br>
						<input type="radio" > Ασφάλιση λόγω νόσου<br>
						<input type="radio" > Παράταση Ασφάλισης<br>
						<input type="radio" > Παράταση Ασφάλισης λόγω νόσου <br>
						</td>
						</tr>
						<?php
								
						echo "<tr>";
						echo "<td style="."padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;".">*του παιδιού μου(Ον/μο): </td>";
						echo '<td> <input  type="text" name="childName" class="demoInputBox" value="';
						if(isset($row->child_name)) echo $row->child_name;
						echo '" maxlength="15" required> </td>';
						echo "</tr>";
						
						echo "<tr>";
						echo "<td style="."padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;".">*που γεννήθηκε την(ημ/νια): </td>";
						echo '<td> <input  type="date" class="demoInputBox" required><td>';
						echo "</tr>";
						
						echo "<tr>";
						echo '<td style="."padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;> -Δηλώνω υπεύθυνα και έγκυρα ότι:το παιδί μου είναι άνεργο, άγαμο, συντηρείται <br>από μένα και δεν είναι ασφ/νο σε άλλο φορέα κι αν στο μέλλον<br> ασφαλιστεί υποχρεούμαι να το δηλώσω μέσα σε ένα μήνα στο Ταμείο. <input  type="checkbox" name="terms" required> <td>';
						echo "</tr>";
								
						echo "<tr>";
						echo "<td style="."padding-left:5px; font-size:13px; font-family:Georgia, Times, serif;"."><td>";
						
						echo '<button name="submit" style="margin:0px -80px;" type="submit" value="HTML" class="btnRegister">Υποβολή</button>
						</form> ';
						echo "</td></td>";
						echo "</tr>";
